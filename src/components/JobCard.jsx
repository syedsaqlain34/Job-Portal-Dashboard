    ...
    <article data-aos="fade-up" className="bg-accent border rounded-lg shadow-card hover:shadow-md transition-shadow overflow-hidden">
      <div className="p-4 flex flex-col sm:flex-row items-start gap-4">
        ...
        <div className="flex-1 w-full">
          ...
          <p className="mt-3 text-sm text-gray-700">{job.description?.slice(0,140)}{job.description && job.description.length > 140 ? '...' : ''}</p>

          <div className="mt-3 flex items-center gap-2 flex-wrap">
            {(job.tags || []).slice(0,4).map((t) => (
              <span key={t} className="text-xs bg-background text-primary px-2 py-1 rounded">{t}</span>
            ))}
          </div>

          <div className="mt-4 flex items-center gap-3">
            <Link to={`/jobs/${job.id}`} className="inline-flex items-center px-3 py-2 bg-secondary text-primary rounded-md text-sm hover:brightness-90">View</Link>
            <a href={job.applyUrl || '#'} target="_blank" rel="noreferrer" className="inline-flex items-center px-3 py-2 border border-gray-200 rounded-md text-sm hover:bg-accent">Apply</a>

            <button onClick={toggleSave} className="ml-auto inline-flex items-center gap-2 px-3 py-2 text-sm rounded-md border border-gray-200" aria-pressed={saved}>
              {saved ? (
